...
考虑在原来的后缀树上，“从一个点 $x$ 开始，往它的子树里面走”这个动作在原字符串上代表什么？当然代表的是从【 $Rootchain(x)$ 所对应的子串】在原串中的**终点集**里的某个位置开始，在原串上往后走。
...
正好，我们可以把前面提到的“后缀链接树”叠加到这张图上，这样图里面有了两类边：来自后缀树的**转移边**，和来自后缀链接的**后缀链接边**。
...
> 
> 假如说我们从点 $x$ 出发，沿着一条字符为 $c$ 的转移边走到了点 $y$，那么在原串上我们相当于从【$x$ 所代表的终点集】中的某一个位置 $a$ 出发，向右走一格后到达一个写着字符 $c$ 的位置，而 $y$ 节点对应的就是包含 $[a-maxlen(x)+2,a+1]$ 这个串的后缀等价类。
> 
